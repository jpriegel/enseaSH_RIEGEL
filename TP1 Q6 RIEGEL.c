...
//cette fois si on tente d'ameliorer l'interface en permettant plus d'argument en entré
int main() {
    ...
    while (1) {
        ...
        if (strncmp("exit", cmd, strlen("exit")) == 0 || cmdlength == 0) {
            ...
            if (ret != 0) { //dans le pere
                wait(&status);
                clock_gettime(clk_id, &stop); //fin du chrono
                if (WIFEXITED(status)) {
                ...
                argv[0] = strtok(cmd, " ");
                int c = 0;
                while (argv[c] != NULL) { //cette boucle permet de prendre en compte tous les arguments exemple "ls -s
                    c++;
                    argv[c] = strtok(NULL, " ");
                }
                clock_gettime(clk_id, &start); // debut du chrono
                execvp(argv[0], argv);
                exit(EXIT_FAILURE);
            }
        }
