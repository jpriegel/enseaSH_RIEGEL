...
//La seule difference ici c'est de pouvoir afficher le msg d'exit soit en ecrivant dans la console soit en ctrl+D
int main() {
    char cmd[CMD_SIZE];
    ...
    while (1) {
        ...
        if (strncmp("exit", cmd, strlen("exit")) == 0 || cmdlength == 0) { //on exit soit en ecrivant soit en ctrl+D
            break; //exit
        } else {
            ret = fork();
